...
function ContentForm({ type, initialData, mode }) {
    ...
    const handleSubmit = async (e)=>{
        ...
        try {
            ...
            if (mode === 'create') {
                const { data: result, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from(table).insert([
                    finalData
                ]).select().single();
                if (error) throw error;
                console.log('Created:', result);
            } else if (initialData?.id) {
                const { data: result, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from(table).update(finalData).eq('id', initialData.id).select().single();
                if (error) throw error;
